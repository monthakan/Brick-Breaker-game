...
LIVES_FONT = pygame.font.SysFont("comicsan", 20)
...
def ball_collistion(ball):
    if ball.x - BALL_RADIUS <= 0 or ball.x + BALL_RADIUS >= WIDTH:
        ball.set_vel(ball.x_vel * -1, ball.y_vel)
    if ball.y + BALL_RADIUS >= HEIGHT or ball.y - BALL_RADIUS<= 0:
        ball.set_vel(ball.x_vel * -1, ball.y_vel * -1)

def ball_paddle_collistion(ball, paddle):
    ...
    for row in range(rows):
        ...
        for col in range(cols):
            brick = Brick(col * brick_width + gap * col, row * brick_height +
                           gap * row , brick_width , brick_height,  1, [(153, 153, 255), (255, 0, 0)])
            bricks.append(brick)
...
def main():
    ...
    while run:
        ...
        ball.move()
        ball_collistion(ball)
        ball_paddle_collistion(ball, paddle)
